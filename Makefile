...
spikes: spikes-offline
	@echo "── auth ──"   && $(MAKE) -s spike-run SPIKE=AuthSpike  SRC="$(API)/GitHubClient.swift $(API)/Models.swift $(API)/RunScope.swift $(API)/ETagStore.swift Sources/Runway/Auth/Keychain.swift" ARGS=check
	@echo "── runs ──"   && $(MAKE) -s spike-run SPIKE=RunsSpike  SRC="$(API)/GitHubClient.swift $(API)/Models.swift $(API)/RunScope.swift $(API)/ETagStore.swift Sources/Runway/Auth/Keychain.swift"
